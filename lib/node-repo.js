...
function getFiles (options, cb) {
  ...
  retry({ times: 5, interval: fiveSeconds }, getGithubFiles, (err, res) => {
    if (err) {
      return options.logger.error(err, 'Error retrieving files from GitHub')
    }

    cb(res.map((fileMeta) => fileMeta.filename))
  })
}

function resolveLabelsThenUpdatePr (options) {
  getFiles(options, function (filepathsChanged) {
...
module.exports = {
  ...
  deferredResolveLabelsThenUpdatePr,
  getFiles,
